...
// IMAGE_FIRST_SECTION doesn't need 32/64 versions since the file header is the same
// either way.
...
#define IMAGE_SCN_NO_DEFER_SPEC_EXC                                                      \
    0x00004000 // Reset speculative exceptions handling bits in the TLB entries for this
               // section.
...
typedef struct IMAGE_COR20_HEADER {
    ...
    // managed entrypoint. If COMIMAGE_FLAGS_NATIVE_ENTRYPOINT is set, EntryPointRVA
    // represents an RVA to a native entrypoint (deprecated for DLLs, use modules
    ...
    // code:AssemblyNative.GetResource and code:PEFile.GetResource and accessible from
    // managed code from System.Assembly.GetManifestResourceStream.  The meta data has a
    // table that maps names to offsets into this blob, so logically the blob is a set of
    // resources.
    ...
    // Used for manged codee that has unmaanaged code inside it (or exports methods as
    // unmanaged entry points)
    IMAGE_DATA_DIRECTORY VTableFixups;
    IMAGE_DATA_DIRECTORY ExportAddressTableJumps;

    // null for ordinary IL images.  NGEN images it points at a code:CORCOMPILE_HEADER
    // structure
